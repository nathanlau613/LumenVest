    ...
    row.innerHTML = `
      <div class="series-main">
        ...
        <div>
          ...
          <div class="series-meta">${escapeHtml(symbolMeta(item.symbol, `${item.points?.length || 0} 点`))}</div>
        </div>
...
function buildGptPayload(entry) {
  ...
  return {
    ...
    range: state.range,
    interval: state.interval,
    currentPrice: lastRawValue(item),
...
function buildReport() {
  const lines = [
    ...
    ...loadedSeries().map((item) => {
      ...
      const quote = state.fundamentals[item.symbol] || {};
      const pe = Number.isFinite(quote.trailingPE) ? quote.trailingPE : quote.forwardPE;
      return `${displaySymbol(item.symbol)}：表现 ${formatValue(lastDisplayValue(item))}，现价 ${formatValue(lastRawValue(item), "price")}，PE ${Number.isFinite(pe) ? pe.toFixed(2) : "不可用"}，AI 观点 ${score.action}(${score.score.toFixed(0)}/100)，RSI ${Number.isFinite(stats.rsi) ? stats.rsi.toFixed(1) : "-"}`;
    }),
    "",
...
els.input.addEventListener("paste", () => {
  window.setTimeout(() => addSymbols(els.input.value), 0);
});

document.querySelectorAll("[data-symbol]").forEach((button) => {
  button.addEventListener("click", () => {
    addSymbols(button.dataset.symbol);
    loadData();
  });
});
