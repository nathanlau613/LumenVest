...
const ipv4Server = createAppServer();
ipv4Server.listen(port, "127.0.0.1", () => {
  console.log(`同步走势已启动：http://127.0.0.1:${port}`);
});

const ipv6Server = createAppServer();
ipv6Server.on("error", (error) => {
  if (error.code !== "EAFNOSUPPORT" && error.code !== "EADDRINUSE" && error.code !== "EPERM") {
    console.error(error);
  }
});
ipv6Server.listen(port, "::1", () => {
  console.log(`同步走势已启动：http://localhost:${port}`);
});
